...
class Corescheduler(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(Detect, '/mbse/Detect', self.core_callback)  # CHANGE

    def core_callback(self, request, response):
        if request.task == 1:
            self.get_logger().info('Incoming Task1 : detect Aruco')  # CHANGE
        return response

def main(args=None):
    rclpy.init(args=args)
    minimal_subscriber = ArucoSubscriber()
    linktocore = Corescheduler()
